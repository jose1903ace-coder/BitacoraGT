// Edge function wp-admin-verify (desplegada en el proyecto Supabase).
// Aprueba o quita la verificación de un proveedor. Solo el administrador
// (ADMIN_EMAIL) puede llamarla; la actualización real la hace la RPC
// wp_admin_set_verified con la clave de servicio.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data, error } = await svc.rpc("wp_admin_set_verified", { p_email: email, p_verified: !!verified });
